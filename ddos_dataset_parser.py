# commands
# Step1: conda create -n python39 python=3.9
# Step2 : conda activate python39
# Step3 : python ddos_dataset_parser.py --dataset_type DOS2019 --dataset_folder ./Dataset/ --packets_per_flow 10 --dataset_id DOS2019 --traffic_type all --time_window 10
# Step4 : python ddos_dataset_parser.py --preprocess_folder Dataset
...
DOS2019_FLOWS = {'attk': ['172.16.0.5'], 'vict': ['192.168.50.1', '192.168.50.4']}
...
def get_ddos_flows(attk,vict):
    DDOS_FLOWS = {}

    if '/' in attk: # subnet
        DDOS_FLOWS['attk'] = [str(ip) for ip in list(ipaddress.IPv4Network(attk).hosts())]
    else: # single address
        DDOS_FLOWS['attk'] = [str(ipaddress.IPv4Address(attk))]

    if '/' in vict:  # subnet
        DDOS_FLOWS['vict'] = [str(ip) for ip in list(ipaddress.IPv4Network(vict).hosts())]
    else:  # single address
        DDOS_FLOWS['vict'] = [str(ipaddress.IPv4Address(vict))]

    return DDOS_FLOWS

def parse_labels(dataset_type=None, attk=None,vict=None, label=1):
    output_dict = {}

    if attk is not None and vict is not None:
        DDOS_FLOWS = get_ddos_flows(attk, vict)
    elif dataset_type is not None and dataset_type in DDOS_ATTACK_SPECS:
        DDOS_FLOWS = DDOS_ATTACK_SPECS[dataset_type]
    else:
        return None

    for attacker in DDOS_FLOWS['attk']:
        for victim in DDOS_FLOWS['vict']:
            ip_src = str(attacker)
            ip_dst = str(victim)
...
def train_test_split(flow_list,train_size=TRAIN_RATE, shuffle=True):
    test_list = []
...
def main(argv):
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        for file in filelist:
            ...
            print("Processing file:", filename)  # 디버그 정보 추가
        ...
        preprocessed_train, preprocessed_test = train_test_split(preprocessed_flows,train_size=TRAIN_RATE, shuffle=True)
        preprocessed_train, preprocessed_val = train_test_split(preprocessed_train, train_size=TRAIN_RATE, shuffle=True)
